  ...
  class InventoriesController < ActionController::API
    ...
    def create
      ...
      params.each do |param|
        ...
        only_prices = inventory_params['only_prices']

        if only_prices
          update_line_items_prices(inventory_params, inventory_line_items_params)
        else
    ...
    def update_line_items_prices(inventory_params, inventory_line_items_params)
      @inventory = Inventory.find_by(unf_number: inventory_params['unf_number'])
      return unless @inventory.present?

      inventory_line_items_params.each { |dt|
        prod = Product.where(unf_id: dt['unf_id'])
        next unless prod.present?

        line_item = @inventory.inventory_line_items.where(product: prod).first
        next unless line_item.present?

        line_item.price = dt['price'].to_f
        line_item.sum = line_item.qty * line_item.price
        line_item.save!
